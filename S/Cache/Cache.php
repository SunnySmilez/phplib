...
class Cache {
    ...
    /**
     * @var \S\Cache\Handler\Abstraction[]
     */
    private static $pools = array();

    public static function __callStatic($name, $args) {
        /**
         * @var \S\Cache\Handler\CacheInterface $cache
         */
    ...
    public static function pool($type, $name = '') {
        $key = self::get_key($type, $name);

        if (!isset(self::$pools[$key])) {
            $handler_ns = __NAMESPACE__ . '\\Handler\\';

            $class = $handler_ns . ucfirst($type);
            /* @var \S\Cache\Handler\CacheInterface $instance */
            $instance = new $class();
            if (!is_subclass_of($instance, $handler_ns . 'CacheInterface')) {
                throw new \Exception($class . ' is not a subclass of \\S\\Cache\\CacheInterface');
            }
    ...
    private static function get_key($type, $name = '') {
