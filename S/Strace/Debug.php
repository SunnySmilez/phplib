...
class Debug {
	...
	protected static function _dump( & $var, $length = 128, $limit = 10, $level = 0)
	{
		...
		elseif (is_resource($var))
		{
			if (($type = get_resource_type($var)) === 'stream' AND $meta = stream_get_meta_data($var))
			{
				...
				if (isset($meta['uri']))
				{
					...
					return '<small>resource</small><span>('.$type.')</span> '.htmlspecialchars($file, ENT_NOQUOTES, \Core\Env::$charset);
				}
			}
			else
			{
		...
		elseif (is_string($var))
		{
			...
			//$var = UTF8::clean($var, Init::$charset);

//			if (UTF8::strlen($var) > $length)
//			{
//				// Encode the truncated string
//				$str = htmlspecialchars(UTF8::substr($var, 0, $length), ENT_NOQUOTES, Init::$charset).'&nbsp;&hellip;';
//			}

			if (mb_strlen($var, \Core\Env::$charset) > $length)
			{
				// Encode the truncated string
				$str = htmlspecialchars(mb_substr($var, 0, $length), ENT_NOQUOTES, \Core\Env::$charset).'&nbsp;&hellip;';
			}
			else
			{
				// Encode the string
				$str = htmlspecialchars($var, ENT_NOQUOTES, \Core\Env::$charset);
			}
		...
		elseif (is_array($var))
		{
			...
			elseif ($level < $limit)
			{
				...
				foreach ($var as $key => & $val)
				{
					...
					if ( ! is_int($key))
					{
						$key = '"'.htmlspecialchars($key, ENT_NOQUOTES, \Core\Env::$charset).'"';
					}
		...
		{
			return '<small>'.gettype($var).'</small> '.htmlspecialchars(print_r($var, TRUE), ENT_NOQUOTES, \Core\Env::$charset);
		}
	}
	 ...
	 *     echo self::path(Kohana::find_file('classes', 'kohana'));
	 *
	...
	public static function source($file, $line_number, $padding = 5)
	{
		...
		while (($row = fgets($file)) !== FALSE)
		{
			...
			if ($line >= $range['start'])
			{
				...
				$row = htmlspecialchars($row, ENT_NOQUOTES, \Core\Env::$charset);
