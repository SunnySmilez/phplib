...
class OTSHandlers
{
    ...
    public function __construct(\OTS\OTSClientConfig $config)
    {
        ...
        } else if (class_exists('\GuzzleHttp\Client')) {
            // Be compatible with Guzzle older version
            $this->httpClient = new \GuzzleHttp\Client($config->getEndPoint());
        } else {
            throw new \OTS\OTSClientException("Internal Error: Faild to create guzzle http client.");
        }
